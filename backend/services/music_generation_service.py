...
DEFAULT_ELEVENLABS_MUSIC_MODEL = "eleven_music_v1"
...
async def _resolve_music_provider() -> tuple[str, str]:
    provider, model_name, _src = await resolve_model(
        "music_generation", "elevenlabs", DEFAULT_ELEVENLABS_MUSIC_MODEL,
    )
    if provider not in PROVIDERS:
        provider = "mock"
    return provider, (model_name or DEFAULT_ELEVENLABS_MUSIC_MODEL)


async def music_real_call_available() -> bool:
    """True if the configured music provider has credentials.
    Plan availability cannot be verified without a real call — use
    `provider_test_service.test_elevenlabs_music()` for that."""
