...
async def ensure_indexes():
    ...
    await db.generation_jobs.create_index("order_id")
    await db.generation_jobs.create_index([("order_id", 1), ("status", 1)])
    await db.generation_jobs.create_index([("order_id", 1), ("job_type", 1)])
    await db.scene_images.create_index([("order_id", 1), ("scene_index", 1)])
